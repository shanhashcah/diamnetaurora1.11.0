...
// validateDiamnetPublicKey returns an error if a public key is invalid. Otherwise, it returns nil.
// It is a wrapper around the IsValidEd25519PublicKey method of the strkey package.
func validateDiamnetPublicKey(publicKey string) error {
	if publicKey == "" {
		return errors.New("public key is undefined")
	}

	if !strkey.IsValidEd25519PublicKey(publicKey) {
		return errors.Errorf("%s is not a valid diamnet public key", publicKey)
	}
	return nil
}

// validateDiamnetSignerKey returns an error if a signerkey is invalid. Otherwise, it returns nil.
func validateDiamnetSignerKey(signerKey string) error {
	if signerKey == "" {
	...
	if err := xdrKey.SetAddress(signerKey); err != nil {
		return errors.Errorf("%s is not a valid diamnet signer key", signerKey)
	}
	return nil
}

// validateDiamnetAsset checks if the asset supplied is a valid diamnet Asset. It returns an error if the asset is
// nil, has an invalid asset code or issuer.
func validateDiamnetAsset(asset Asset) error {
	if asset == nil {
	...
	err = validateDiamnetPublicKey(asset.GetIssuer())
	if err != nil {
...
// validateAmount checks if the provided value is a valid diamnet amount, it returns an error if not.
// This is used to validate price and amount fields in structs.
func validateAmount(n interface{}) error {
	var diamnetAmount int64
...
func validateAllowTrustAsset(asset Asset) error {
	// Note: we are not using validateDiamnetAsset() function for AllowTrust operations because it requires the
	//  following :
	// - asset is non-native
	// - asset code is valid
...
func validateChangeTrustAsset(asset Asset) error {
	// Note: we are not using validateDiamnetAsset() function for ChangeTrust operations because it requires the
	//  following :
	// - asset is non-native
	// - asset code is valid
...
// It checks that the buying and selling assets are valid diamnet assets, and that amount and price are valid.
...
func validatePassiveOffer(buying, selling Asset, offerAmount, price string) error {
	...
	// https://github.com/diamnet/go/pull/1707#discussion_r321508440
	err := validateDiamnetAsset(buying)
	if err != nil {
		return NewValidationError("Buying", err.Error())
	}

	err = validateDiamnetAsset(selling)
	if err != nil {
...
// It checks that the buying and selling assets are valid diamnet assets, and that amount, price and offerID
...
// https://github.com/diamnet/diamnet-protocol/blob/master/ecosystem/sep-0011.md#asset
