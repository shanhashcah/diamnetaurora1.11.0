...
func TestValidateDiamnetPublicKey(t *testing.T) {
	validKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7YFMTLB65PYM"
	err := validateDiamnetPublicKey(validKey)
	assert.NoError(t, err, "public key should be valid")

	invalidKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y"
	err = validateDiamnetPublicKey(invalidKey)
	expectedErrMsg := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y is not a valid diamnet public key"
	...
	err = validateDiamnetPublicKey(invalidKey)
	expectedErrMsg = "SBCVMMCBEDB64TVJZFYJOJAERZC4YVVUOE6SYR2Y76CBTENGUSGWRRVO is not a valid diamnet public key"
	require.EqualError(t, err, expectedErrMsg, "public key should be invalid")
}

func TestValidateDiamnetAssetWithValidAsset(t *testing.T) {
	nativeAsset := NativeAsset{}
	err := validateDiamnetAsset(nativeAsset)
	assert.NoError(t, err)

	kp0 := newKeypair0()
	creditAsset := CreditAsset{"XYZ", kp0.Address()}
	err = validateDiamnetAsset(creditAsset)
	assert.NoError(t, err)
}

func TestValidateDiamnetAssetWithInValidAsset(t *testing.T) {
	err := validateDiamnetAsset(nil)
	assert.Error(t, err)
	...
	err = validateDiamnetAsset(creditAssetNoCode)
	assert.Error(t, err)
	...
	err = validateDiamnetAsset(creditAssetNoIssuer)
	assert.Error(t, err)
...
func TestValidateAmountInvalidValue(t *testing.T) {
	...
	require.EqualError(t, err, expectedErrMsg, "should be a valid diamnet amount")
