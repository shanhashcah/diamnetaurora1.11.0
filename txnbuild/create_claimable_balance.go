...
// CreateClaimableBalance represents the Diamnet create claimable balance operation. See
...
func (cb *CreateClaimableBalance) Validate() error {
	for _, d := range cb.Destinations {
		err := validateDiamnetPublicKey(d.Destination)
		if err != nil {
	...
	err = validateDiamnetAsset(cb.Asset)
	if err != nil {
