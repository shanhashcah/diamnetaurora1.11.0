...
// MinBaseFee is the minimum transaction fee for the Diamnet network.
...
func stringsToKP(keys ...string) ([]*keypair.Full, error) {
	...
	for _, k := range keys {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "provided string %s is not a valid Diamnet key", k)
		}
		kpf, ok := kp.(*keypair.Full)
		if !ok {
			return nil, errors.New("provided string %s is not a valid Diamnet secret key")
		}
...
func (t *Transaction) ClaimableBalanceID(operationIndex int) (string, error) {
	...
	// We mimic the relevant code from Diamnet Core
	// https://github.com/diamnet/diamnet-core/blob/9f3cc04e6ec02c38974c42545a86cdc79809252b/src/test/TestAccount.cpp#L285
...
// verifyTxSignature checks if a transaction has been signed by the provided Diamnet account.
