...
func Reset(client *auroraclient.Client, keys []Account) {
...
// first account.
func Initialise(client *auroraclient.Client, keys []Account) {
...
// from Aurora. This code demonstrates how to retrieve and inspect the error.
func TXError(client *auroraclient.Client, keys []Account) {
	keys = loadAccounts(client, keys)
...
func loadAccounts(client *auroraclient.Client, accounts []Account) []Account {
...
// loadAccount is an example of how to get an account's details from Aurora.
func loadAccount(client *auroraclient.Client, address string) *hProtocol.Account {
	accountRequest := auroraclient.AccountRequest{AccountID: address}
	auroraSourceAccount, err := client.AccountDetail(accountRequest)
	if err != nil {
...
func submit(client *auroraclient.Client, txeBase64 string) (resp hProtocol.Transaction) {
	...
	if err != nil {
		...
		err = printAuroraError(hError)
		dieIfError("couldn't print Aurora eror", err)
		os.Exit(1)
	}
...
// printAuroraError is an example of how to inspect the error returned from Aurora.
func printAuroraError(hError *auroraclient.Error) error {
