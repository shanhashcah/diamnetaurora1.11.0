...
func TestOperationBodyGoStringerSetOptions(t *testing.T) {
	operationBody := xdr.OperationBody{
		...
		SetOptionsOp: &xdr.SetOptionsOp{
			...
			HomeDomain:    xdr.String32Ptr("diamnet.org"),
			Signer: &xdr.Signer{
...
func TestOperationBodyGoStringerAllowTrust(t *testing.T) {
	operationBody := xdr.OperationBody{
		...
		AllowTrustOp: &xdr.AllowTrustOp{
			...
			Asset:     xdr.MustNewAllowTrustAsset("USD"),
			Authorize: 1,
		},
	}
	...
	assert.Equal(
		t,
		`xdr.OperationBody{Type: xdr.OperationTypeAllowTrust,AllowTrustOp: &xdr.AllowTrustOp{Trustor:xdr.MustAddress("GC7ERFCD7QLDFRSEPLYB3GYSWX6GYMCHLDL45N4S5Q2N5EJDOMOJ63V4"), Asset:xdr.MustNewAllowTrustAsset("USD"), Authorize:1}}`,
		fmt.Sprintf("%#v", operationBody),
	)
}
