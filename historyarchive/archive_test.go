...
func GetTestS3Archive() *Archive {
	...
	bucket := fmt.Sprintf("s3://history-stg.diamnet.org/dev/archivist/test-%s", r)
	region := "eu-west-1"
		...
		return ioutil.NopCloser(strings.NewReader(`
{
	...
	"networkPassphrase": "Public Global Diamnet Network ; September 2015"
}`))
	}
		...
		return ioutil.NopCloser(strings.NewReader(`
{
	...
	assert.NoError(t, err)
	_, err = archive.GetPathHAS("has.json")
	assert.EqualError(t, err, "Network passphrase does not match! expected=Test SDF Network ; September 2015 actual=Public Global Diamnet Network ; September 2015")
}
