...
# one-time signer being consumed
  ...
  key = Diamnet::SignerKey.hash_x(x)
  set_options :onetime, signer: Diamnet::Signer.new({
    key: key, 
    weight: 1,
  })
  close_ledger #4
  ...
  tx = Diamnet::Transaction.manage_data({
    account:  account,
  ...
  env = tx.to_envelope
  env.signatures << Diamnet::DecoratedSignature.new({
