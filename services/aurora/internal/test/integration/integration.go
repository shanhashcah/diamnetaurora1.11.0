...
import (
	...
	sdk "github.com/diamnet/go/clients/auroraclient"
	"github.com/diamnet/go/clients/diamnetcore"
	"github.com/diamnet/go/keypair"
	proto "github.com/diamnet/go/protocols/aurora"
...
const (
	...
	diamnetCorePostgresPassword = "integration-tests-password"
	adminPort                   = 6060
)

var (
	diamnetCorePort         = mustPort("tcp", "11626")
	diamnetCorePostgresPort = mustPort("tcp", "5432")
...
func NewTest(t *testing.T, config Config) *Test {
	...
	image := "diamnet/quickstart:testing"
	...
	diamnetCoreBinding := containerInfo.NetworkSettings.Ports[diamnetCorePort][0]
	coreURL := fmt.Sprintf("http://%s:%s", diamnetCoreBinding.HostIP, diamnetCoreBinding.HostPort)
	...
	i.cclient = &diamnetcore.Client{URL: coreURL}
...
func (i *Test) startAurora(containerInfo types.ContainerJSON, coreURL string) {
	diamnetCorePostgres := containerInfo.NetworkSettings.Ports[diamnetCorePostgresPort][0]
	diamnetCorePostgresURL := fmt.Sprintf(
		"postgres://diamnet:%s@%s:%s/core",
		diamnetCorePostgresPassword,
		diamnetCorePostgres.HostIP,
		diamnetCorePostgres.HostPort,
	)
...
// closed by Diamnet-Core. Panics in case of errors. Note it's different
...
func createTestContainer(i *Test, image string) error {
	...
	if err != nil {
		...
		args.Add("reference", "diamnet/quickstart:testing")
	...
	if os.Getenv("HORIZON_INTEGRATION_ENABLE_CAPTIVE_CORE") != "" {
		containerConfig.Env = append(containerConfig.Env,
			...
			"DIAMNET_CORE_BINARY_PATH=/opt/diamnet/core/bin/start",
			"DIAMNET_CORE_CONFIG_PATH=/opt/diamnet/core/etc/diamnet-core.cfg",
		)
		...
		containerConfig.Env = append(containerConfig.Env,
			"POSTGRES_PASSWORD="+diamnetCorePostgresPassword,
		)
		containerConfig.ExposedPorts = nat.PortSet{
			diamnetCorePort:         struct{}{},
			diamnetCorePostgresPort: struct{}{},
...
func (i *Test) RunAuroraCLICommand(cmd ...string) {
	fullCmd := append([]string{"/diamnet/aurora/bin/aurora"}, cmd...)
