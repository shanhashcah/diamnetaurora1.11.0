...
func initExpIngester(app *App) {
	...
	if !app.config.EnableCaptiveCoreIngestion {
		coreSession = mustNewDBSession(app.config.DiamnetCoreDatabaseURL, ingest.MaxDBConnections, ingest.MaxDBConnections)
	}
	app.ingester, err = ingest.NewSystem(ingest.Config{
		...
		DiamnetCoreBinaryPath:    app.config.DiamnetCoreBinaryPath,
		DiamnetCoreConfigPath:    app.config.DiamnetCoreConfigPath,
...
func initPathFinder(app *App) {
	...
	app.orderBookStream = ingest.NewOrderBookStream(
		&history.Q{app.AuroraSession(app.ctx)},
		orderBookGraph,
	)
...
func initDbMetrics(app *App) {
	...
	app.dbMaxOpenConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "aurora", Subsystem: "db", Name: "max_open_connections"},
		func() float64 {
			// Right now MaxOpenConnections in Aurora is static however it's possible that
	...
	app.dbOpenConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "aurora", Subsystem: "db", Name: "open_connections"},
		func() float64 {
	...
	app.dbInUseConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "aurora", Subsystem: "db", Name: "in_use_connections"},
		func() float64 {
	...
	app.dbWaitCountCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{
			Namespace: "aurora", Subsystem: "db", Name: "wait_count_total",
	...
	app.dbWaitDurationCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{
			Namespace: "aurora", Subsystem: "db", Name: "wait_duration_seconds_total",
...
func initSubmissionSystem(app *App) {
	app.submitter = &txsub.System{
		...
		DB: func(ctx context.Context) txsub.AuroraDB {
			return &history.Q{Session: app.AuroraSession(ctx)}
		},
	}
}
