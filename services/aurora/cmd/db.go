...
var dbReapCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		app := aurora.NewAppFromFlags(config, flags)
...
var dbReingestRangeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		ingestConfig := ingest.Config{
			...
			DiamnetCoreBinaryPath:       config.DiamnetCoreBinaryPath,
			RemoteCaptiveCoreURL:        config.RemoteCaptiveCoreURL,
		}

		if !ingestConfig.EnableCaptiveCore {
			if config.DiamnetCoreDatabaseURL == "" {
				log.Fatalf("flag --%s cannot be empty", aurora.DiamnetCoreDBURLFlagName)
			}
			coreSession, dbErr := db.Open("postgres", config.DiamnetCoreDatabaseURL)
			if dbErr != nil {
		...
		if errors.Cause(err) == ingest.ErrReingestRangeConflict {
			message := `
			The range you have provided overlaps with Aurora's most recently ingested ledger.
