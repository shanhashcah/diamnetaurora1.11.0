...
// diamnet-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with diamnet-core.  If nil,
...
// SetCursor calls the `setcursor` command on the connected diamnet core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on diamnet-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected diamnet core with the provided envelope
...
// WaitForNetworkSync continually polls the connected diamnet-core until it
...
// simpleGet returns a new GET request to the connected diamnet-core using the
