			...
			for {
				...
				if err != nil {
					if err == io.EOF || err == io.ErrUnexpectedEOF {
						...
						//   happen in Aurora as it always sends an empty line after each event.
...
// See https://www.diamnet.org/developers/aurora/reference/endpoints/accounts.html
...
// See https://www.diamnet.org/developers/aurora/reference/endpoints/offer-details.html
...
// See https://www.diamnet.org/developers/aurora/reference/resources/operation.html
func (c *Client) OperationDetail(id string) (ops operations.Operation, err error) {
	...
	if err != nil {
		return ops, errors.Wrap(err, "sending request to aurora")
	}
...
// Paths returns the available paths to make a strict receive path payment. See https://www.diamnet.org/developers/aurora/reference/endpoints/path-finding-strict-receive.html
...
// StrictReceivePaths returns the available paths to make a strict receive path payment. See https://www.diamnet.org/developers/aurora/reference/endpoints/path-finding-strict-receive.html
func (c *Client) StrictReceivePaths(request PathsRequest) (paths hProtocol.PathsPage, err error) {
	err = c.sendRequest(request, &paths)
	return
}

// StrictSendPaths returns the available paths to make a strict send path payment. See https://www.diamnet.org/developers/aurora/reference/endpoints/path-finding-strict-send.html
...
// StreamEffects streams aurora effects. It can be used to stream all effects or account specific effects.
...
// FetchTimebounds provides timebounds for N seconds from now using the server time of the aurora instance.
...
func (c *Client) FetchTimebounds(seconds int64) (txnbuild.Timebounds, error) {
	serverURL, err := url.Parse(c.AuroraURL)
	if err != nil {
