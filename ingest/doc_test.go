...
func Example_ledgerentrieshistoryarchive() {
	archiveURL := "http://history.diamnet.org/prd/core-live/core_live_001"
...
// for a specific ledger using captive diamnet-core. Please note that transaction
// meta IS available when using this backend.
func Example_changes() {
	archiveURL := "http://history.diamnet.org/prd/core-live/core_live_001"
	networkPassphrase := network.PublicNetworkPassphrase

	// Requires Diamnet-Core 13.2.0+
	backend, err := ledgerbackend.NewCaptive(
		"/bin/diamnet-core",
		"/opt/diamnet-core.cfg",
		networkPassphrase,
		[]string{archiveURL},
	)
	if err != nil {
