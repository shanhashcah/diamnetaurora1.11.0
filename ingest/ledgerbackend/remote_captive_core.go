...
// RemoteCaptiveDiamnetCore is an http client for interacting with a remote captive core server.
type RemoteCaptiveDiamnetCore struct {
	url                      *url.URL
	client                   *http.Client
	lock                     *sync.Mutex
...
// RemoteCaptiveOption values can be passed into NewRemoteCaptive to customize a RemoteCaptiveDiamnetCore instance.
type RemoteCaptiveOption func(c *RemoteCaptiveDiamnetCore)
...
func PrepareRangePollInterval(d time.Duration) RemoteCaptiveOption {
	return func(c *RemoteCaptiveDiamnetCore) {
		c.prepareRangePollInterval = d
	}
}

// NewRemoteCaptive returns a new RemoteCaptiveDiamnetCore instance.
//
// Only the captiveCoreURL parameter is required.
func NewRemoteCaptive(captiveCoreURL string, options ...RemoteCaptiveOption) (RemoteCaptiveDiamnetCore, error) {
	u, err := url.Parse(captiveCoreURL)
	if err != nil {
		return RemoteCaptiveDiamnetCore{}, errors.Wrap(err, "unparseable url")
	}

	client := RemoteCaptiveDiamnetCore{
...
// in the backend.
func (c RemoteCaptiveDiamnetCore) GetLatestLedgerSequence() (sequence uint32, err error) {
	u := *c.url
...
func (c RemoteCaptiveDiamnetCore) Close() error {
	c.lock.Lock()
	defer c.lock.Unlock()
	if c.cancel != nil {
		c.cancel()
...
func (c RemoteCaptiveDiamnetCore) createContext() context.Context {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c RemoteCaptiveDiamnetCore) PrepareRange(ledgerRange Range) error {
	ctx := c.createContext()
	u := *c.url
...
func (c RemoteCaptiveDiamnetCore) IsPrepared(ledgerRange Range) (bool, error) {
	u := *c.url
	u.Path = path.Join(u.Path, "prepare-range")
	rangeBytes, err := json.Marshal(ledgerRange)
	if err != nil {
...
func (c RemoteCaptiveDiamnetCore) GetLedger(sequence uint32) (bool, xdr.LedgerCloseMeta, error) {
	u := *c.url
