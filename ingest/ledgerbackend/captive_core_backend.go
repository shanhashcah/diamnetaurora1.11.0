...
// changes are implemented in Diamnet-Core.
//
...
type CaptiveDiamnetCore struct {
	...
	// For testing
	diamnetCoreRunnerFactory func(configPath string) (diamnetCoreRunnerInterface, error)
	...
	// for scenarios when Aurora consumes ledgers faster than Diamnet-Core produces them
...
// NewCaptive returns a new CaptiveDiamnetCore.
//
// All parameters are required, except configPath which is not required when
// working with BoundedRanges only.
func NewCaptive(executablePath, configPath, networkPassphrase string, historyURLs []string) (*CaptiveDiamnetCore, error) {
	...
	return &CaptiveDiamnetCore{
		archive:           archive,
		executablePath:    executablePath,
		configPath:        configPath,
		historyURLs:       historyURLs,
		networkPassphrase: networkPassphrase,
		diamnetCoreRunnerFactory: func(configPath2 string) (diamnetCoreRunnerInterface, error) {
			return newDiamnetCoreRunner(executablePath, configPath2, networkPassphrase, historyURLs)
		},
...
func (c *CaptiveDiamnetCore) openOfflineReplaySubprocess(from, to uint32) error {
	...
	if c.diamnetCoreRunner == nil {
		...
		c.diamnetCoreRunner, err = c.diamnetCoreRunnerFactory("")
		if err != nil {
...
func (c *CaptiveDiamnetCore) openOnlineReplaySubprocess(from uint32) error {
	...
	if c.diamnetCoreRunner == nil {
		if c.configPath == "" {
			return errors.New("diamnet-core config file path cannot be empty in an online mode")
		}
		c.diamnetCoreRunner, err = c.diamnetCoreRunnerFactory(c.configPath)
		if err != nil {
			return errors.Wrap(err, "error creating diamnet-core runner")
		}
	}
...
func (c *CaptiveDiamnetCore) runFromParams(from uint32) (runFrom uint32, ledgerHash string, nextLedger uint32, err error) {
	if from == 1 {
	...
	if from <= 63 {
		...
		nextLedger = from - 63
		if nextLedger < 2 {
			// Diamnet-Core always streams from ledger 2 at min.
			nextLedger = 2
		}
	}
...
func (c *CaptiveDiamnetCore) sendLedgerMeta(untilSequence uint32) {
	defer c.wait.Done()
	...
	for {
		...
		if err != nil {
			select {
			case processErr := <-c.diamnetCoreRunner.getProcessExitChan():
				...
				if processErr != nil {
					err = errors.Wrap(processErr, "diamnet-core process exited with an error")
				} else {
					err = errors.New("diamnet-core process exited without an error unexpectedly")
				}
			default:
			}
...
func (c *CaptiveDiamnetCore) readLedgerMetaFromPipe() (*xdr.LedgerCloseMeta, error) {
	metaPipe := c.diamnetCoreRunner.getMetaPipe()
	if metaPipe == nil {
...
func (c *CaptiveDiamnetCore) PrepareRange(ledgerRange Range) error {
	...
	for {
		...
		if len(c.metaC) > 0 {
			...
			if c.processExit {
				if c.processErr != nil {
					err = errors.Wrap(c.processErr, "diamnet-core process exited with an error")
				} else {
					err = errors.New("diamnet-core process exited without an error unexpectedly")
				}
			}
...
func (c *CaptiveDiamnetCore) IsPrepared(ledgerRange Range) (bool, error) {
	if c.nextLedger == 0 {
		return false, nil
	}
...
//
// Because data is streamed from Diamnet-Core ledger after ledger user should
...
func (c *CaptiveDiamnetCore) GetLedger(sequence uint32) (bool, xdr.LedgerCloseMeta, error) {
...
// in the backend.
func (c *CaptiveDiamnetCore) GetLatestLedgerSequence() (uint32, error) {
	if c.isClosed() {
...
func (c *CaptiveDiamnetCore) Close() error {
	...
	if c.diamnetCoreRunner != nil {
		err := c.diamnetCoreRunner.close()
		c.diamnetCoreRunner = nil
		if err != nil {
			return errors.Wrap(err, "error closing diamnet-core subprocess")
		}
	}
