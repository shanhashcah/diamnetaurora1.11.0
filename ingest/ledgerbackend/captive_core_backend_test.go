...
type diamnetCoreRunnerMock struct {
	mock.Mock
}

func (m *diamnetCoreRunnerMock) catchup(from, to uint32) error {
	a := m.Called(from, to)
	return a.Error(0)
}

func (m *diamnetCoreRunnerMock) runFrom(from uint32, hash string) error {
	a := m.Called(from, hash)
	return a.Error(0)
}

func (m *diamnetCoreRunnerMock) getMetaPipe() io.Reader {
	a := m.Called()
	return a.Get(0).(io.Reader)
}

func (m *diamnetCoreRunnerMock) getProcessExitChan() <-chan error {
	a := m.Called()
	return a.Get(0).(chan error)
}

func (m *diamnetCoreRunnerMock) close() error {
	a := m.Called()
	return a.Error(0)
}
...
func TestCaptiveNew(t *testing.T) {
	executablePath := "/etc/diamnet-core"
	configPath := "/etc/diamnet-core.cfg"
	networkPassphrase := network.PublicNetworkPassphrase
	historyURLs := []string{"http://history.diamnet.org/prd/core-live/core_live_001"}

	captiveDiamnetCore, err := NewCaptive(
		executablePath,
		configPath,
		networkPassphrase,
		historyURLs,
	...
	assert.Equal(t, executablePath, captiveDiamnetCore.executablePath)
	assert.Equal(t, configPath, captiveDiamnetCore.configPath)
	assert.Equal(t, networkPassphrase, captiveDiamnetCore.networkPassphrase)
	assert.Equal(t, historyURLs, captiveDiamnetCore.historyURLs)
	assert.Equal(t, uint32(0), captiveDiamnetCore.nextLedger)
	assert.NotNil(t, captiveDiamnetCore.archive)
}
...
func TestCaptivePrepareRange(t *testing.T) {
	...
	mockRunner := &diamnetCoreRunnerMock{}
	...
	captiveBackend := CaptiveDiamnetCore{
		archive:           mockArchive,
		networkPassphrase: network.PublicNetworkPassphrase,
...
func TestCaptivePrepareRangeCrash(t *testing.T) {
	...
	err := captiveBackend.PrepareRange(BoundedRange(100, 200))
	assert.Error(t, err)
	assert.EqualError(t, err, "diamnet-core process exited with an error: exit code -1")
}
...
func TestCaptivePrepareRangeTerminated(t *testing.T) {
	...
	err := captiveBackend.PrepareRange(BoundedRange(100, 200))
	assert.Error(t, err)
	assert.EqualError(t, err, "diamnet-core process exited without an error unexpectedly")
}
...
func TestCaptivePrepareRange_ErrClosingSession(t *testing.T) {
	...
	captiveBackend := CaptiveDiamnetCore{
		networkPassphrase: network.PublicNetworkPassphrase,
		nextLedger:        300,
		diamnetCoreRunner: mockRunner,
	}

	err := captiveBackend.PrepareRange(BoundedRange(100, 200))
	assert.EqualError(t, err, "opening subprocess: error closing existing session: error closing diamnet-core subprocess: transient error")
}
...
func TestCaptivePrepareRange_ErrCatchup(t *testing.T) {
	...
	err := captiveBackend.PrepareRange(BoundedRange(100, 200))
	assert.Error(t, err)
	assert.EqualError(t, err, "opening subprocess: error running diamnet-core: transient error")
}
...
func TestCaptiveGetLedger_ErrClosingAfterLastLedger(t *testing.T) {
	...
	_, _, err = captiveBackend.GetLedger(66)
	tt.EqualError(err, "error closing session: error closing diamnet-core subprocess: transient error")
}

func waitForBufferToFill(captiveBackend *CaptiveDiamnetCore) {
	for {
