...
import (
	...
	slog "github.com/diamnet/go/support/log"
...
var rootCmd = &cobra.Command{
	Use:   "aurora-cmp",
	Short: "aurora-cmp compares two aurora servers' responses",
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&auroraBase, "base", "b", "", "URL of the base/old version Aurora server")
	rootCmd.PersistentFlags().StringVarP(&auroraTest, "test", "t", "", "URL of the test/new version Aurora server")
...
func run(cmd *cobra.Command) {
	...
	ledger := getLatestLedger(auroraBase)
	...
	outputDir := fmt.Sprintf("%s/aurora-cmp-diff/%s", pwd, time.Now().Format(timeFormat))
	...
	for {
		...
		go func() {
			...
			go func() {
				a = cmp.NewResponse(auroraBase, pl.Path, pl.Stream)
				requestWg.Done()
			}()
			go func() {
				b = cmp.NewResponse(auroraTest, pl.Path, pl.Stream)
				requestWg.Done()
			}()
			...
			// older Aurora versions don't send this header.
...
func getLatestLedger(url string) protocol.Ledger {
	aurora := client.Client{
		AuroraURL: url,
		HTTP:       http.DefaultClient,
	}

	ledgers, err := aurora.Ledgers(client.LedgerRequest{
...
func addPathsFromResponse(a *cmp.Response, level int) {
	...
	for _, newPath := range newPaths {
		...
		// without cursor. There will always be a diff if Aurora started
